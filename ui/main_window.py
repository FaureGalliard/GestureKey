...
_PANEL_WIDTH    = 240
_SELECTING_TEXT = "Selecting camera…"
_LOADING_TEXT   = "Loading camera"
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self._console.setMaximumWidth(0)         
        self._console.hide()
        ...
        self._console_anim = QPropertyAnimation(self._console, b"maximumWidth", self)
        self._console_anim.setDuration(250)
        self._console_anim.setEasingCurve(QEasingCurve.Type.InOutCubic)
        self._console_anim.valueChanged.connect(self._reposition_toolbar)
        self._console_anim.finished.connect(self._on_anim_finished)
    ...
    def _reposition_toolbar(self) -> None:
        ...
        if cw is None:
            return
        rect      = cw.rect()
        top_left  = cw.mapTo(self, rect.topLeft())
        top_right = cw.mapTo(self, rect.topRight())
    ...
    def on_frame(self, frame: np.ndarray) -> None:
        ...
        if not self._size_set:
            h, w = frame.shape[:2]
            self.resize(min(w, 1280), min(h, 800))
            self._size_set = True
        self._camera_view.update_frame(frame)

    def on_camera_stopped(self) -> None:
        self._camera_view.show_overlay(_SELECTING_TEXT, animate=False)

    def on_camera_loading(self) -> None:
        self._camera_view.show_overlay(_LOADING_TEXT, animate=True)
