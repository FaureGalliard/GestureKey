...
class PinchZoomGesture(Gesture):
    ...
    def detect(self, frame_data: FrameData) -> List[GestureEvent]:
        ...
        if self._smoothed_center is None:
            ...
            self._smoothed_center = (
                POSITION_ALPHA * center_raw[0]
                + (1 - POSITION_ALPHA) * self._smoothed_center[0],
                POSITION_ALPHA * center_raw[1]
                + (1 - POSITION_ALPHA) * self._smoothed_center[1],
            )
        ...
        if self._prev_pinch_raw is not None:
            ...
            if abs(delta) > MOTION_THRESHOLD:
                ...
                and now - self._last_motion_time > STILLNESS_TIMEOUT
            ):
                self._prev_pinch_raw = pinch_dist_raw
                return events
    ...
    def _valid_detection(self, center, hand_size, pinch) -> bool:
        ...
        if (
            hand_size
            and self._last_valid_size
            and self._last_valid_size > 0
            and abs(hand_size - self._last_valid_size)
            / self._last_valid_size
            > MAX_SIZE_CHANGE
        ):
            return False
        if (
            pinch
            and self._last_valid_pinch
