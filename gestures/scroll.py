...
MAX_SCROLL_STEP = 100000   # límite de seguridad
...
VELOCITY_DEADZONE = 0.002   # umbral de velocidad mínima para scroll

# 🥈 INTEGRADOR TEMPORAL (suaviza dirección)
TEMPORAL_INTEGRATION_FRAMES = 3  # frames a acumular antes de scroll
DY_BUFFER_SIZE = 3               # tamaño del buffer

# 🥉 DETECCIÓN DE QUIETUD (mano quieta = no scroll)
MOTION_THRESHOLD = 0.003    # movimiento mínimo para considerar "en movimiento"
STILLNESS_TIMEOUT = 0.1     # 100ms sin movimiento = quieto


class ScrollGesture:
    """Scroll tipo touchpad robusto con protección contra errores de detección"""
    ...
    def detect(self, state, main_hand, hand_landmarks_raw=None):
        ...
        if not self.scroll_active:
            ...
            if not is_valid_detection:
                ...
                if self.outlier_count > OUTLIER_RECOVERY_FRAMES:
                    print("[SCROLL] ⚠️ Demasiados outliers pre-armado - reseteando")
                    self._reset()
        ...
        if not self.scroll_active:
            ...
            print("[SCROLL] ✅ Armado y activo")

        # =====================
        # SCROLL CON DY CRUDO (sin suavizado)
        # =====================
        if self.prev_center_raw is not None:
            ...
            if abs(dy_normalized) > MOTION_THRESHOLD:
                ...
                if self.last_motion_time and now - self.last_motion_time > STILLNESS_TIMEOUT:
                    # mano quieta por >100ms → no scroll
                    self.prev_center_raw = center_raw
                    return events
            ...
            if self.position_100ms_ago is None or self.time_100ms_ago is None:
                ...
                if total_displacement < MOTION_THRESHOLD:
                    # mano prácticamente quieta → no scroll
                    self.prev_center_raw = center_raw
                    return events
            ...
            scroll_amount = int(dy_effective * gain)
    ...
    def _validate_detection(self, center, hand_size):
        ...
        if position_jump > MAX_POSITION_JUMP:
            print(f"[SCROLL] ⚠️ Salto de posición: {position_jump:.3f}")
            return False
        ...
        if hand_size is not None and self.last_valid_size is not None:
            ...
            if size_change_ratio > MAX_SIZE_CHANGE:
                print(f"[SCROLL] ⚠️ Cambio de tamaño: {size_change_ratio:.2%}")
                return False
